...
// equals bag[where]; otherwise, anEntry is not in the array
    ...
    public void removeDuplicates() {
        ...
        for (int i = 0; i < numberOfEntries; i++) {
            T currentEntry = bag[i];
            
            for (int j = i + 1; j < numberOfEntries; j++) {
                if (currentEntry.equals(bag[j])) {
                    removeEntry(j);
                    j--; // Decrement to adjust for the shift in the array after removal
                }
            }
        }
    ...
    public boolean addAll(BagInterface<T> toAdd) {
        ...
        T[] arrayToAdd = toAdd.toArray();
        for (T item : arrayToAdd) {
            if (!add(item)) {
                success = false;  // If the bag overflows, return false and stop adding
                break;
            }
        }
    ...
    for (int i = 0; i < numberOfEntries; i++) {
        for (int j = i + 1; j < numberOfEntries; j++) {
            if (bag[i].equals(bag[j])) {
                return false;  // If any duplicate is found, it's not a set
            }
        }
    }
...
public T getMode() {
    ...
    for (int i = 0; i < numberOfEntries; i++) {
        int currentCount = getFrequencyOf(bag[i]);
        
        if (currentCount > maxCount) {
            maxCount = currentCount;
            mode = bag[i];
        } else if (currentCount == maxCount) {
            mode = null;  // If there’s a tie, return null
        }
    }
